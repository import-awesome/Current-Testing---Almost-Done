				...
				if event.type == pygame.KEYDOWN:
					if event.key == pygame.K_z:
						...
						if self.char_y == self.winy / 2 + 70:
							...
							living[selection].defend = 1
							self.b.battlequeue.append([living[selection], None, 4])
							selection += 1

						if self.char_y == self.winy / 2 + 105:
							print "Run"
							small = pygame.font.Font(None, 36)
							runchance = random.randint(0, 100) % 50
							self.b.battlequeue.append([living[selection], self.b.enemies, 3])
					...
					if event.key == pygame.K_DOWN:
						if self.b.enemies[0].boss == 1:
							if self.char_y != self.winy / 2 + 70:
								self.char_y += 35
						elif self.char_y != self.winy / 2 + 105:
							self.char_y += 35
			...
			if selection >= len(living):
				...
				for x in self.b.battlequeue:
					...
					if x[2] == 3:
						...
						ran = 0
						ran = runaway(self.screen, self.winx, self.winy, x[0], x[1], x[2])
						print ran
						if ran == 1:
							run = True
							display = False
...
def runaway(screen, winx, winy, character, enemy, direction):

	if direction == 3:
		if enemy[0].boss != 1:
			small = pygame.font.Font(None, 36)
			act = "{} tries to run away.".format(character.name)
			act2 = "{} failed to run.".format(character.name)
			act3 = "{} ran.".format(character.name)
...
def battlecalc(screen, winx, winy, character, enemy, direction, action = 0):
	...
	if direction == 4:
		defe = "{} defends.".format(character.name)
		character.stats[2] += 4
		defe_t = small.render(defe, 1, WHITE)
		while framecounter < 90:
			pygame.draw.rect(screen, WHITE, (winx/4 - 35, winy/2+80, 450, 40))
			pygame.draw.rect(screen, GREY, (winx/4 - 33, winy/2+82, 446, 36))

			if framecounter < 45:
				screen.blit(defe_t, (winx/4 -30, winy/2+85))
		...
		if action == 0:
			...
			while framecounter < 90:
				...
				if framecounter < 45:
					...
					if character.hp - int(math.ceil((3*enemy.stats[0] - character.physdefence) * math.sqrt(enemy.stats[0]/character.stats[2]) * rand)) <= 0:
						screen.blit(act3_t, (winx/4 - 30, winy/2+85))
					else:	
						screen.blit(act2_t, (winx/4 - 30, winy/2+85))
		...
		if action == 0:
			...
			while framecounter < 90:
				...
				if framecounter < 45:
					...
					if enemy.hp - int(math.ceil((character.physattk - enemy.stats[2]) * math.ceil(math.sqrt(character.stats[0]/enemy.stats[2])) * rand)) <= 0:
						screen.blit(act3_t, (winx/4 - 30, winy/2+85))
					else:
						screen.blit(act2_t, (winx/4 - 30, winy/2+85))
