...
class Boss(object):
	def __init__(self, screen, p_x, p_y, x, y):
		...
		self.x = x
		self.y = y
		pygame.draw.rect(self.screen, PURP, (p_x, p_y, x, y))
...
class ShowMaze(object):
	...
	def fogshow(self, curx, cury, dir):
		...
		if self.map[cury][curx] == B or self.map[cury][curx] == V:
			...
			if self.map[cury][curx] == V:
				Boss(self.screen, math.floor(curx * self.winx / 35), math.floor(cury * self.winy / 30), math.floor(self.winx/35), math.floor(self.winy/30))
	...
	def Display(self):
		...
		for x in self.map:
			for y in x:
				...
				if y == V:
					Boss(self.screen, math.floor(r * self.winx / 35), math.floor(c * self.winy / 30), math.floor(self.winx/35), math.floor(self.winy/30))
